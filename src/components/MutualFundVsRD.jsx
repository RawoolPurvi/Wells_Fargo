...
import useMutualFundVsRD from '../hooks/useMutualFundVsRD';

const MutualFundVsRD = () => {
  const [location, setLocation] = useState("New York - Manhattan");
  const [startDate, setStartDate] = useState("2023-04-01");
  const [endDate, setEndDate] = useState("2023-06-30");
  const [category, setCategory] = useState("Gold");
  const [data, setData] = useState([]);
  const fetchData = useMutualFundVsRD;

  const handleFetchData = () => {
    const result = fetchData(location, startDate, endDate);
    setData(result);
  };
